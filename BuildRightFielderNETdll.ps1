# Name:    BuildRightFielderNETdll
# Purpose: Compile the mdRightFielderNET.dll wrapper
...
Write-Host "`n======================= WELCOME TO MELISSA RIGHT FIELDER OBJECT NET WRAPPER =======================`n"
...
cmd.exe /C """$CmdPath"" x86_x64 && csc -out:mdRightFielderNET.dll -platform:AnyCPU -target:library mdRightFielder_cSharpCode.cs" > $null

if (Test-Path ("mdRightFielderNET.dll")) {
  $filePath = Join-Path -Path (Split-Path -Parent $PSCommandPath) -ChildPath "mdRightFielderNET.dll"

  Write-Host "`nmdRightFielderNET.dll has been successfully generated at:`n$filePath`n"
}
else {
  Write-Host "`nError: mdRightFielderNET.dll was not created successfully`n"
